...
class sct_finder(object):
	...
	def __init__(self, _magma_group_name, _group_tag):
		...
		self._sct_list = [] # This is a list of SCTs as partitions of Irr(G)
		self._superclass_list = [] # list of SCTs as partitions of Cl(G)
		# Magma likes to return the conjugacy classes in (seemingly) random orders for some groups.  This magma code saves eveything sage needs up front in text files to avoid some issues caused by this.
		S =	'CharacterTableToMatrix:=function(G) '\
		+		'CT:=CharacterTable(G);'\
		+		'K:=CyclotomicField(LCM([CyclotomicOrder(CoefficientField(CT[i])):i in [1..#CT]]));'\
		+		'return [[K!CT[i,j]: j in [1..#CT]]: i in [1..#CT]];'\
		+	'end function;'\
		+	'PrintFileMagma("table",CharacterTableToMatrix(___GROUP___));'\
		+	'PrintFileMagma("gens",Generators(___GROUP___));'\
		+	'PrintFileMagma("classes",ConjugacyClasses(___GROUP___));'
		...
		# Save a sage-friendly variable representing the character table.
		f = open('table','r')
		the_string = f.read()
		f.close
		CT = magma(the_string)
			...
			for j in range(self._N):
				if 'z' in X[i][j]:
					...
					if '(' in X[i][j]:
						...
						order = str(magma.CyclotomicOrder(magma.Parent(CT[i+1][j+1])))
						X[i][j] = X[i][j].replace('zeta_'+order, 'E('+order+')')
		...
		# More magma code for generating the lists of automorphic and Galois SCTs up front.  Later, these will be compared against the list of all SCTs. 
		S =	'load "gens";'\
		+	'gens:=$1;'\
		+	'G:=sub<___GROUP___|gens>;'\
		+	'A:=AutomorphismGroup(G);'\
		+	'R:=CharacterRing(G);'\
		+	'load "table";'\
		+	'CT:=$1;'\
		+	'CT:=[[CT[i,j]:j in [1..#CT]]:i in [1..#CT]];'\
		+	'CT:=[R!x:x in CT];'\
		+	'X:=SequenceToSet(CT);'\
		+	'load "classes";'\
		+	'Cl:=$1;'\
		+	'gens:=SetToSequence(Generators(A));'\
		+	'f:=map<CartesianProduct(X,A)->X|x:->R![(map<G->CoefficientField(x[1])|y:->x[1](x[2](y))>)(G!(Cl[i,3])):i in [1..#Cl]]>;'\
		+	'P:=PermutationGroup<{1..#CT}|[[Position(CT,f(CT[i],gens[j])):i in [1..#CT]]: j in [1..#gens]]>;'\
		+	'PrintFileMagma("autpermgroup",P);'\
		+	'sg:=Subgroups(P);'\
		+	'sgps:=[x`subgroup:x in sg];'\
		+	'subgps:=[];'\
		+	'for i in [1..#sgps] do '\
		+		'list:=SetToSequence(Class(P,sgps[i]));'\
		+		'subgps:=subgps cat list;'\
		+	'end for;'\
		+	'PrintFileMagma("autscts",SetToSequence(SequenceToSet([Sort([Sort([Orbits(subgps[i])[j,k]:k in [1..#(Orbits(subgps[i])[j])]]):j in [1..#Orbits(subgps[i])]]):i in [1..#subgps]])));'\
		+	'K:=CyclotomicField(LCM([CyclotomicOrder(CoefficientField(CT[i])):i in [1..#CT]]));'\
		+	'n:=CyclotomicOrder(K);'\
		+	'list:=[];'\
		+	'all:=[i:i in [1..n-1]];'\
		...
		+	'sg:=Subgroups(P);'\
		+	'sgps:=[x`subgroup:x in sg];subgps:=[];'\
		+	'for i in [1..#sgps] do '\
		+		'list:=SetToSequence(Class(P,sgps[i]));'\
		+		'subgps:=subgps cat list;'\
		+	'end for;'\
		+	'PrintFileMagma("galoisscts",SetToSequence(SequenceToSet([Sort([Sort([Orbits(subgps[i])[j,k]:k in [1..#(Orbits(subgps[i])[j])]]):j in [1..#Orbits(subgps[i])]]):i in [1..#subgps]])));'
		S = S.replace('___GROUP___',self._magma_group_name)
		magma.eval(S)

		# The image of Aut(G) in Sym(Cl(G)) (which is what autpermgroup represents) has one more use later to check SCTs for characteristicity.
		f = open('autpermgroup','r')
		the_string = f.read()
		f.close()
		the_string = the_string.replace('\n','').replace('\r','')
		the_string += ';'
		g = open('autpermgroup','w')
		g.write(the_string)
		g.close()
	...
	def export_data(self):
		...
		os.remove('classes')
		os.remove('autscts')
		os.remove('galoisscts')
		os.remove('autpermgroup')
	...
	def _is_characteristic_sct(self, partition):
		...
		S =	'load "autpermgroup";'\
		+	'P:=$1;'\
		+	'is_characteristic_sct:=function(P,S) '\
		+		'O:=SetToSequence(Orbit(P,S));'\
		+		'n:=#SequenceToSet([Sort([Sort(O[i,j]):j in [1..#O[i]]]): i in [1..#O]]);'\
		+		'if n eq 1 then '\
		+			'return true;'\
		+		'else '\
		+			'return false;'\
		+		'end if;'\
		+	'end function;'\
		+	'is_characteristic_sct(P,___P___)'
	...
	def _check_scts_are_characteristic(self):
		...
		for i in range(len(self._sct_list)):
			if self._is_characteristic_sct(self._sct_list[i]):
	...
	def _check_scts_are_galois(self):
		# This method compares elements of sct_list against the list of Galois SCTs computed earlier.
		f= open('galoisscts')
		S =  f.read()
		f.close()
		S = magma.eval(S)
	...
	def _check_scts_are_automorphic(self):
		# This method compares elements of sct_list against the list of automorphic SCTs computed earlier.
		f= open('autscts')
		S =  f.read()
		f.close()
		S = magma.eval(S)
